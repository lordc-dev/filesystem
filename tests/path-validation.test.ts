...
describe("parseFileUri EACCES/EPERM", () => {
  it("validatePath resolves valid paths", async () => {
    const { validatePath } = await import("../src/validation/path-validation.js");
    const tmpFile = path.join(os.tmpdir(), "mcp-path-validation-test.txt");
    await fs.writeFile(tmpFile, "test", "utf-8");
    try {
      const result = await validatePath(tmpFile);
      expect(result).toContain("mcp-path-validation-test.txt");
    } finally {
